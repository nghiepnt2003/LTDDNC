...
const { sendMail, sendMailRegister } = require("../../util/sendMail");
class UserController {
  ...
  //[GET] /registerSendOTP/
  async registerSendOTP(req, res, next) {
    try {
            ...
            <body>
                <div class="container">
                    <div class="content">
                        <h1>Book Store</h1>
                        <p>Xin chào,</p>
                        <p>Đây là OTP để tạo tài khoản của bạn.</p>
      ...
      const result = await sendMailRegister(data);
  ...
  //[GET] /registerSendOTP/
  async editSendOTP(req, res, next) {
    try {
      const { email } = req.query;
      if (!email) throw new Error("Missing email");
      let user = await User.findOne({ email });
